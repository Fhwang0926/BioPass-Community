...
const AUTH_TEXT = {
  ko: {
    ...
    authChoicePushFirebaseDisabled: '서버에 Firebase가 설정되지 않아 앱으로 알림을 보낼 수 없습니다. Bio Pass 앱을 열고 요청 현황을 새로고침하면 이 요청이 표시됩니다. 또는 이메일 보안 코드로 인증해 주세요.',
    ...
    js: {
      ...
      bioFirebaseDisabled: 'Firebase가 설정되지 않아 앱으로 알림을 보낼 수 없습니다. Bio Pass 앱에서 요청 현황을 새로고침하거나 이메일 인증을 이용해 주세요.',
  ...
  en: {
    ...
    authChoicePushFirebaseDisabled: 'This server has not configured Firebase, so it cannot send app notifications. Open Bio Pass and refresh the request list to see this request, or use an email security code.',
    ...
    js: {
      ...
      bioFirebaseDisabled: 'Firebase is not configured on this server, so app notifications cannot be sent. Refresh the request list in Bio Pass, or use email verification.',

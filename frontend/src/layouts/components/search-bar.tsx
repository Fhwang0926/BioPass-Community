...
export default function SearchBar() {
	...
	// 라벨이 있는 라우트만 검색 대상 (빈 label은 t() missingKey 방지)
	const routesWithLabel = useMemo(
		() => flattenedRoutes.filter((item) => !!item.label),
		[flattenedRoutes]
	);

	const searchResult = useMemo(() => {
		return routesWithLabel.filter(
			(item) => {
					...
					<Scrollbar>
						<div ref={listRef} className="py-2">
							{searchResult.map(({ key, label }, index) => {
								const labelText = label ? t(label) : key;
								const labelStr = typeof labelText === "string" ? labelText : String(labelText);
								const partsTitle = parse(labelStr, match(labelStr, searchQuery));
